...
Since there are thousands genes in the dataset, to get strong ones, the threshold is set to 1*10^-7.
...
versionDF <- versionDF[versionDF$annotation_data %in% c("GO:BP.version", "REAC.version", "WP.version"), ]
versionDF <- versionDF[, c(2, 1)]
versionDF$annotation_data <- c("GO:BP", "REAC", "WP")
row.names(versionDF) <- 1:3
versionDF %>%
  kableExtra::kbl(caption = "Annotation Data", ) %>%
  kableExtra::kable_classic(full_width = F, html_font = "Cambria")
ranked <- read.delim("CRC_ranked_genelist.txt")
colnames(ranked) <- c("V1","V2")
ranked <- unique(ranked)
qlf_output_hits_withgn <- gprofiler2::gost(query = c(ranked$V1),
                            organism = "hsapiens", 
...
g:Profiler is used due to its easy and user friendly annotating tool.

2. What annotation data did you use and why? What version of the annota"on are you using?
3. How many genesets were returned with what thresholds?
4. Run the analysis using the up-regulated set of genes, and the down-regulated set of genes separately. How do
these results compare to using the whole list (i.e all differen"ally expressed genes together vs. the up-regulated
and down regulated differen"ally expressed genes separately)?
