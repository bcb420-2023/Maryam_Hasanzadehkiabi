...
if(min(heatmap_matrix_hgnc_symbol) == 0){
heatmap_col = colorRamp2(c( 0, max(heatmap_matrix_hgnc_symbol)),
c( "white", "red"))
} else {
...
length(which(qlf_output_hits$table$PValue < 0.05))# number of genes with p-value < 0.05
length(which(qlf_output_hits$table$FDR < 0.05)) # number of genes with adjusted p_value (FDR) < 0.05
...
row.names = FALSE,col.names = FALSE,quote = FALSE)
write.table(x=data.frame(genename= qlf_output_hits_withgn$gname,F_stat= qlf_output_hits_withgn
